...
#1. Load the data
acc_mask = torch.load(r"/root/autodl-tmp/MOBA-T2mapping-master/dataset/ACC6 uniform,24,93-116 are 1.pth")    #(256,208,12,8)
kspace_data = torch.load(r"/root/autodl-tmp/MOBA-T2mapping-master/dataset/rotated_kdata.pth")[:,:,selected_list,:].to(torch.complex64)#(256,208,8,12)

print(torch.max(torch.abs(kspace_data)))
...
sensitivity_maps = scio.loadmat(r"/root/autodl-tmp/MOBA-T2mapping-master/dataset/coil_sensitivities.mat")['sens']
sensitivity_maps = torch.from_numpy(sensitivity_maps).to(torch.complex64)      #(256,208,12)
#acc_mask = torch.from_numpy(scio.loadmat(r"/root/autodl-tmp/MOBA-T2mapping-master/dataset/ACC6, masks/ACC6, no ACS.mat")['mask_all'])
...
all_trn_mask, loss_mask = torch.empty((num_reps, H, W,n_coil,N_time)), torch.empty((num_reps, H, W,n_coil,N_time))
...
val_ref_kspace = torch.empty(kspace_data.shape,dtype=torch.complex64).to(device)  #(256,208,31,12)
val_input_k = torch.empty(kspace_data.shape,dtype=torch.complex64).to(device)     #(256,208,31,12)

trn_ref_kspace = torch.empty((num_reps, H, W, N_time, n_coil),dtype=torch.complex64).to(device)  # ref_kspace:(25,256,208,31,12),Loss
trn_input_k = torch.empty((num_reps, H, W, N_time, n_coil), dtype=torch.complex64).to(device)  # (25,256,208,31,12)
...
sensitivity_maps = torch.tile(sensitivity_maps.unsqueeze(0),(num_reps,1,1,1))                 #(25,256,208,12)
...
directory = os.path.join('TE<200,scaled6acc_acs_pretrain_data', str(acc_rate)+'Rate' + '_' + str(pretrain_iter) + 'Pretrain_iter')
