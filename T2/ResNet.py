...
def conv_layer(filter_size, padding = 1, is_instance_norm = False, activation_type='ReLU',*args,**kwargs):
    kernel_size, in_c, out_c = filter_size
    return nn.Sequential(nn.Conv2d(in_channels=in_c,out_channels=out_c,kernel_size=kernel_size,padding=padding,bias=False),InstanceNorm(is_instance_norm,in_c),
...
class ResNet(nn.Module):
    def __init__(self,device,in_ch = 2, num_of_resblocks=5):
        ...
        self.r2layer = nn.ReLU(inplace=False)
    ...
    def forward(self,input_x):
        ...
        r_out = self.r2layer(nw_out[:,2:,...])         #Ensure R is non-negative
        final_out = torch.cat([nw_out[:,:2,...],r_out],dim=1)
