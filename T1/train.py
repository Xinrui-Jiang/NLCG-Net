...
sensitivity_maps = torch.tile(sensitivity_maps.unsqueeze(0),(num_reps,1,1,1))                 #(25,192,158,32), block, if it has been applied formerly

#2. Load criterion reference
ref_r1 = torch.load(os.path.join(data_dirt,"ref_M_R1.pth"))[...,2].to(device)
brain_mask = torch.load(os.path.join(data_dirt,"T1cfsmask_11.pth")).to(torch.float32).to(device)
...
norm_k = torch.max(torch.abs(kspace_data))
print('max value: {}'.format(norm_k))
kspace_data = kspace_data / norm_k
kspace_data = kspace_data.permute(0,1,3,2)                              #192,158,5,32

#4. Load pretrain data, which are masks, data after 800 iteration for initialization
data_directory = r"/root/autodl-tmp/MOBA-T2mapping-master/T1 mapping pretrain data/2Rate_800Pretrain_iter/"
...
model_directory = os.path.join('T1 recon_result', model_name+'not changeloss,Unetout_as_param,ACC'+str(acc_rate)+'LR'+ str(learning_rate)+'CG iter'+str(CG_iter) +'Unrolls'+str(nb_unroll_blocks))
...
best_checkpoint = torch.load(r"/root/autodl-tmp/MOBA-T2mapping-master/T1 recon_result/3layerUnetACC4LR0.0001CG iter20Unrolls3/best.pth")
...
while ep < epochs and val_loss_tracker < stop_training:
    ...
    tic = time.time()
    trn_loss, lamdas,trn_nrmse,trn_imgnrmse = train(train_loader, model, loss_fn, optimizer, ep,ref_r1,brain_mask,norm_k=norm_k,device=device)
    total_train_loss.append(trn_loss)
    trn_nrmse_record.append(trn_nrmse)
    trn_imgnrmse_record.append(trn_imgnrmse)
    
    val_loss,val_nrmse,val_imgnrmse = validation(val_loader, model, loss_fn,ref_r1,brain_mask,norm_k=norm_k, device=device)
...
torch.save(model,os.path.join(model_directory,'model.pth'))
